...
export function AddCourseImage() {
  ...
  const { form, onSubmit, courseId, isPending, existingImageUrl } = useCourseImageForm();
          ...
          render={({ field, fieldState }) => (
            <Field data-invalid={!!fieldState.error}>
              <FieldLabel className="font-cairo-semibold-base text-greyDarker">
                {t('label')} <span className="text-red-500 ms-1">*</span>
              </FieldLabel>
              
              <div 
                onClick={() => fileInputRef.current?.click()}
                className={`mt-2 flex flex-col items-center justify-center w-full h-64 border-2 border-dashed rounded-xl cursor-pointer transition-colors ${
              ...
              >
                {displayImage ? (
                  ...
                  <img src={displayImage} alt="Preview" className="w-full h-full object-contain rounded-xl p-2" />
                ) : (

...
export const updateCourseStatus = async (courseId: string, newStatus: number) => {
  try {
    ...
    if (!response.ok) {
      ...
      return { serverError: errorData?.message || 'Failed to update course status. Please try again.' };
    }

    const resultData: IApiResponse<boolean> = await response.json();
    if (!resultData.isSuccess) {
      return { serverError: resultData.message || 'Failed to update course status' };
    }

...
export const getSectionsByCourseId = async (courseId: string) => {
  try {
    ...
    if (!response.ok) {
      ...
      return { serverError: errorData?.message || 'Failed to fetch sections.' };
    }

    const resultData: IApiResponse<SectionDto[]> = await response.json();
    if (!resultData.isSuccess) {
      return { serverError: resultData.message || 'Failed to fetch sections' };
    }
...
export const createSection = async (data: CreateSectionRequest) => {
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => null);
      return { serverError: errorData?.message || 'Failed to create section.' };
    }

    const resultData: IApiResponse<string> = await response.json();
    if (!resultData.isSuccess) {
      return { serverError: resultData.message || 'Failed to create section' };
    }
...
export const updateSection = async ({ id, data }: { id: string; data: UpdateSectionRequest }) => {
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => null);
      return { serverError: errorData?.message || 'Failed to update section.' };
    }

    const resultData: IApiResponse<null> = await response.json();
    if (!resultData.isSuccess) {
      return { serverError: resultData.message || 'Failed to update section' };
    }
...
export const deleteSection = async (id: string) => {
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => null);
      return { serverError: errorData?.message || 'Failed to delete section.' };
    }

    const resultData: IApiResponse<null> = await response.json();
    if (!resultData.isSuccess) {
      return { serverError: resultData.message || 'Failed to delete section' };
    }

...
export const updatePreviewVideos = async (courseId: string, data: CreatePreviewVideosPayload) => {
  try {
    ...
    if (!response.ok) {
      ...
      return { serverError: errorData?.message || 'Failed to update preview videos. Please try again.' };
    }

    const resultData: IApiResponse<string> = await response.json();
    if (!resultData.isSuccess) {
      return { serverError: resultData.message || 'Failed to update preview videos' };
    }
...
export const deletePreviewVideo = async (courseId: string, videoId: string) => {
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => null);
      return { serverError: errorData?.message || 'Failed to delete preview video. Please try again.' };
    }

    const resultData: IApiResponse<string> = await response.json();
    if (!resultData.isSuccess) {
      return { serverError: resultData.message || 'Failed to delete preview video' };
    }

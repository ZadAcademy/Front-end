...
export const updateCoursePreview = async (courseId: string, canPreview: boolean) => {
  try {
    ...
    if (!response.ok) {
      ...
      return { serverError: errorData?.message || 'Failed to update course preview. Please try again.' };
    }

    const resultData: IApiResponse<boolean> = await response.json();
    if (!resultData.isSuccess) {
      return { serverError: resultData.message || 'Failed to update course preview' };
    }

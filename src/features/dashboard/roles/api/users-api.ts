...
export async function getUsers(params: GetUsersQueryParams = {}) {
  try {
    ...
    if (!response.ok) {
      ...
      return { serverError: errorData?.message || 'Failed to fetch users' };
    }

    const resultData: IApiResponse<PaginatedResult<UserWithRoles>> = await response.json();
    if (!resultData.isSuccess) {
      return { serverError: resultData.message || 'Failed to fetch users' };
    }
